...
function(){
  ...
  assign("traitlth",NA, envir=.rqtlnetwork)
  ...
  lyout[1,1]<- gbutton("QNK File...", cont = lyout, handler = function(h,...) {
      ...
      if(std != "") {
        ...
        assign("qnkfile",x, envir=.rqtlnetwork)
        assign("traitlth",y, envir=.rqtlnetwork)
        assign("traitname",z, envir=.rqtlnetwork)
  ...
  lyout[2,1]<-gbutton("Map File...", cont=lyout,
        ...
        if(std != "")
        {
          ...
          assign("mapfile",x, envir=.rqtlnetwork)
          assign("chromosome",y, envir=.rqtlnetwork)
  ...
  lyout[1,1]<-gbutton("Trait...", cont=lyout,
    handler = function(h,...)
    {
        ...
        {
            allow<-AllowTrait()
            std<-myselect.list(list=get("traitname",envir=.rqtlnetwork)[allow],multiple=FALSE)
  ...
  lyout[1,3]<-gbutton("Radii",cont=lyout)
  lyout[1,4]<-(widgets$radii<-gedit("11",cont=lyout))
  lyout[2,1]<-gbutton("ShowMarker",cont=lyout)
  lyout[2,2]<-(widgets$showMarker<-gcombobox(c("Yes","No"),cont=lyout))
  lyout[2,3]<-gbutton("ShowValue",cont=lyout)
  lyout[2,4]<-(widgets$showValue<-gcombobox(c("Yes","No"),cont=lyout))
  lyout[3,1]<-gbutton("MarkerPosition",cont=lyout)
  lyout[3,2]<-(widgets$markerPosition<-gedit("10",cont=lyout))
  lyout[3,3]<-gbutton("ValuePosition",cont=lyout)
  lyout[3,4]<-(widgets$valuePosition<-gedit("10",cont=lyout))
  lyout[4,1]<-gbutton("MarkerFont",cont=lyout)
  lyout[4,2]<-(widgets$markerFont<-gedit("1",cont=lyout))
  lyout[4,3]<-gbutton("MarkerFontSize",cont=lyout)
  lyout[4,4]<-(widgets$markerFontSize<-gedit("0.6",cont=lyout))
  lyout[5,1]<-gbutton("ValueFont",cont=lyout)
  lyout[5,2]<-(widgets$valuerFont<-gedit("1",cont=lyout))
  lyout[5,3]<-gbutton("ValueFontSize",cont=lyout)
  lyout[5,4]<-(widgets$valueFontSize<-gedit("0.6",cont=lyout))
  lyout[6,1]<-gbutton("ChromosomeWidth",cont=lyout)
  lyout[6,2]<-(widgets$chrWidth<-gedit("15",cont=lyout))
  lyout[6,3]<-gbutton("EpistasisLineWidth",cont=lyout)
  lyout[6,4]<-(widgets$epiLineWidth<-gedit("1",cont=lyout))
  lyout[7,1]<-gbutton("ChromosomeFont",cont=lyout)
  lyout[7,2]<-(widgets$chrFont<-gedit("2",cont=lyout))
  lyout[7,3]<-gbutton("ChromosomeFontSize",cont=lyout)
  lyout[7,4]<-(widgets$chrFontSize<-gedit("1",cont=lyout))
  lyout[8,1]<-gbutton("NotationFont",cont=lyout)
  ...
  AllowTrait<-function()
  {
      ...
      traitlth<-get("traitlth",envir=.rqtlnetwork)
      #print(traitlth)
  ...
  MIPlot<-function(){
      ...
      #traitVal<-which(is.element(strsplit(svalue(widgets$trait),split=", ")[[1]],get("traitname",envir=.rqtlnetwork)))
  ...
  QTLeffectPlot<-function(){
      ...
      #trait<-which(is.element(strsplit(svalue(widgets$Effectrait),split=", ")[[1]],get("traitname",envir=.rqtlnetwork)))
      #print(get("traitname",envir=.rqtlnetwork))
      ...
      if(MinY!="") {Miny<-as.numeric(MinY)
                        ...
                        if(length(Dpos)!=1) {
                        MinY<-floor(min(Avalue,AEvalue))
                        }else{
                        MinY<-floor(min(Avalue,AEvalue,Dvalue,DEvalue))
                        }
                    }
      if(MaxY!="") {MaxY<-as.numeric(MaxY)
                    }else{
                        if(length(Dpos)!=1) {
                        MaxY<-ceiling(max(Avalue,AEvalue))
                        }else{
                        MaxY<-ceiling(max(Avalue,AEvalue,Dvalue,DEvalue))
                        }
                    }
      ...
      for(i in 1:length(AEpos)){
          for(j in 1:length(AEpvalue[,1])){
              if(AEpvalue[j,i]) {grid.points(x=unit(1:length(Avalue)-posA,"native")[j],y=unit(AEvalue[j,i],"native"),
                                pch=pchIndex[i],gp=gpar(col=color[i], fill=color[i]))
                                }else{
                                grid.points(x=unit(1:length(Avalue)-posA,"native")[j],y=unit(AEvalue[j,i],"native"),
  ...
  QTLNetworkPlot<-function(){
    #trait<-which(is.element(strsplit(svalue(widgets$QTLNtrait),split=", ")[[1]],get("traitname",envir=.rqtlnetwork)))
    trait<-as.numeric(lapply(strsplit(svalue(widgets$QTLNtrait),split=", ")[[1]],function(item){
                x<-grep(item,get("traitname",envir=.rqtlnetwork))
            }))
    #trait<-as.numeric(svalue(widgets$QTLNtrait))
    radii<-as.numeric(svalue(widgets$radii))/1000
    showMarker<-svalue(widgets$showMarker)
    showValue<-svalue(widgets$showValue)
    markerPosition<-as.numeric(svalue(widgets$markerPosition))/1000
    valuePosition<-as.numeric(svalue(widgets$valuePosition))/1000
    markerFont<-as.numeric(svalue(widgets$markerFont))
    markerFontSize<-as.numeric(svalue(widgets$markerFontSize))
    valueFont<-as.numeric(svalue(widgets$valueFont))
    valueFontSize<-as.numeric(svalue(widgets$valueFontSize))
    ...
    if(length(trait)>2) stop("Too much traits.")
  ...
  #if(!is.na(match(trait,allow_trait))){
  ##find same chr info in different traits
    trait1_Qechr<-check_Qechr[1:(Qe_end+1-Qe_start)[1]]
    trait2_Qechr<-check_Qechr[((Qe_end+1-Qe_start)[1]+1):sum(Qe_end-Qe_start+1)]
    #samechr1<-which(is.element(trait1_Qechr,trait2_Qechr))
    #samechr2<-which(is.element(trait2_Qechr,trait1_Qechr))+(Qe_end+1-Qe_start)[1]
    ...
    if(length(dup_Qeidx)>0){
        ...
        col_A<-col_A[-dup_Qeidx]
        check_Qechr<-check_Qechr[-dup_Qeidx]
        Qe_chr_info<-Qe_chr_info[-dup_Qeidx]
        Qe_pos_st<-Qe_pos_st[-dup_Qeidx]
        Qe_pos_ed<-Qe_pos_ed[-dup_Qeidx]
        Qe_mkst<-Qe_mkst[-dup_Qeidx]
        Qe_mked<-Qe_mked[-dup_Qeidx]
        Qe_pos<-Qe_pos[-dup_Qeidx]
        Qe_pos_lth<-Qe_pos_lth[-dup_Qeidx]
        if(length(trait)>1){
        traitinfo1<-traitinfo1[-dup_Qeidx]
        }
    }
  ...
  if(length(traitWithEe)>0){
    #Ee info
    Ee_start<-grep("_epistasis_effect",qnkfile[,1])[trait[traitWithEe]]+2
    Ee_end<-grep("_epistasis_heritability",qnkfile[,1])[trait[traitWithEe]]-1
    ...
    dup_idx<-which(duplicated(check_chr))
    uniq_chr<-unique(check_chr)
    if(length(dup_idx)>0){
        pos_st<-as.numeric(bind(Qe_pos_st,Ee_pos_st)[-dup_idx])
        pos_ed<-as.numeric(bind(Qe_pos_ed,Ee_pos_ed)[-dup_idx])
        pos<-as.numeric(bind(Qe_pos,Ee_pos)[-dup_idx])
        pos_lth<-bind(Qe_pos_lth,Ee_pos_lth)[-dup_idx]
        mkst<-bind(Qe_mkst,Ee_mkst)[-dup_idx]
        mked<-bind(Qe_mked,Ee_mked)[-dup_idx]
        chr_info<-as.numeric(bind(Qe_chr_info,Ee_chr_info)[-dup_idx])
    }else{
    ...
    if(length(trait)>1){
        ...
        #if(length(dupinfo)>0){
         #   pre_dupidx<-which(is.element(uniq_chr,dupinfo))
        #}
        if(length(dup_idx)>0) traitinfo<-as.character(traitinfo[-dup_idx])
        if(length(pre_dupidx)>0) traitinfo[pre_dupidx]<-paste(trait[1],trait[2],sep=",")#toString(trait)
    }



    #####
    if(length(uniq_chr)>length(col_A)){
        col_A[(length(col_A)+1):length(uniq_chr)]<-"black"
        if(!is.na(Qe_Dpos[1]))  col_D[(length(col_D)+1):length(uniq_chr)]<-"black"
    }
    ...
    chr_length<-0
    for(i in 1:max(chr_info)){
       drawchr<-0
       u<-1
       for(j in 1:length(chr_info)){
          if(chr_info[j]==i) {
          drawchr[u]<-j
          u<-u+1
          if(is.na(chr_length[i])) chr_length[i]<-pos_lth[j] else chr_length[i]<-chr_length[i]+pos_lth[j]
          }
       }
    }
    del_chr_lth<-chr_length[!is.na(chr_length)]
    ##
    col_unit<-1/(length(del_chr_lth)+1)
    row_unit<-(0.85-0.05*length(which(chr_info==which.max(chr_length))))/max(del_chr_lth)
    poslth_stead<-pos_lth
    pos_lth[pos_lth/max(del_chr_lth)<0.07]<-0.07*max(del_chr_lth)
    #legend
    ...
    y1<-rep(unit(0.75,"npc"),time=m)
    y2<-rep(unit(0.25,"npc"),time=n)
    if(!is.na(Qe_Dpos[1])){
        mytextD<-c("D","DE","D+DE")
        upD<-unit(0.1+(1:m+2)*0.14,"npc")
        grid.rect(x=upD,y=y1,gp=gpar(col=NA,fill=mycol),just="left",
                  height=unit(0.2,"npc"),width=unit(0.02,"npc"))
        grid.text(mytextD,x=upD+unit(0.03,"npc"),y=y1,just="left",
            gp=gpar(cex=notationFontSize,font=notationFont))
    }
    grid.circle(x=up,y=y1,r=0.1,gp=gpar(col=NA,fill=mycol))
    for(i in 1:n){
        grid.segments(x0=down[i],x1=down[i]+unit(0.05,"npc"),
                   y0=y2[i],y1=y2[i],
                   gp=gpar(col=mycol[i],lty="dashed"))
    }
    grid.text(mytextA,x=up+unit(0.02,"npc"),y=y1,just="left",
            gp=gpar(cex=notationFontSize,font=notationFont))
    grid.text(mytextI,x=down+unit(0.07,"npc"),y=y2,just="left",
            gp=gpar(cex=notationFontSize,font=notationFont))
    popViewport()
    pushViewport(viewport(layout.pos.row=2,name="row2"))
    ...
    Ee_xpos<-Ee_ypos<-0
    for(i in 1:max(chr_info)){
       if(!is.na(chr_length[i])){
       k<-k+1
       grid.text(paste("CH",i,sep=""),x=unit(-1.2,"lines"),
                       y=unit(col_unit*k,"npc"),
                       gp=gpar(cex=chrFontSize,font=chrFont))
       drawst<-0.04
       drawchr<-which(chr_info==i)
       ord<-order(pos[drawchr])
       for(j in 1:length(drawchr)){
           if(j>1) drawst<-drawst+0.07+pos_lth[drawchr][ord][j-1]*row_unit
           grid.rect(x=unit((drawst+pos_lth[drawchr][ord][j]*row_unit+drawst)/2,"npc"),
                              y=unit(col_unit*k,"npc"),
                              width=unit(pos_lth[drawchr][ord][j]*row_unit,"npc"),
                              height=unit(chrWidth,"npc"),
                              gp=gpar(col="darkgrey",fill="darkgrey"))
                    grid.circle(x=drawst+row_unit*(pos[drawchr][ord][j]-pos_st[drawchr][ord][j])/poslth_stead[drawchr][ord][j]*pos_lth[drawchr][ord][j],
                                y=col_unit*k,r=radii,
                         gp=gpar(fill=col_A[drawchr][ord][j],col=col_A[drawchr][ord][j]))
                    if(length(dup_Qeidx)>0&length(which(pre_dupidx==drawchr[ord][j]))>0) grid.semiCircle(x=drawst+row_unit*(pos[drawchr][ord][j]-pos_st[drawchr][ord][j])/poslth_stead[drawchr][ord][j]*pos_lth[drawchr][ord][j],
                                y=col_unit*k,radii=radii,col=col_dup[which(pre_dupidx==drawchr[ord][j])],fill=col_dup[which(pre_dupidx==drawchr[ord][j])])
                    if(length(trait)>1){
                        grid.text(traitinfo[drawchr][ord][j],x=drawst+row_unit*(pos[drawchr][ord][j]-pos_st[drawchr][ord][j])/poslth_stead[drawchr][ord][j]*pos_lth[drawchr][ord][j],
                                  y=col_unit*k,
                                  gp=gpar(col="white",cex=0.5))
                    }
                    if(showValue=="Yes"){
                        grid.text(pos_st[drawchr][ord][j],x=unit(drawst,"npc"),
                                      y=unit(col_unit*k-valuePosition,"npc"),
                                      just=c("center","top"),
                                      gp=gpar(cex=valueFontSize,font=valueFont))
                        grid.text(pos_ed[drawchr][ord][j],
                                      x=unit(pos_lth[drawchr][ord][j]*row_unit+drawst,"npc"),
                                      y=unit(col_unit*k-valuePosition,"npc"),
                                      just=c("center","top"),
                                      gp=gpar(cex=valueFontSize,font=valueFont))
                    }
                    if(showMarker=="Yes"){
                        grid.text(mkst[drawchr][ord][j],x=unit(drawst,"npc"),
                                      y=unit(markerPosition+col_unit*k,"npc"),
                                      just=c("center","bottom"),
                                      gp=gpar(cex=markerFontSize,font=markerFont))
                        grid.text(mked[drawchr][ord][j],
                                      x=unit(pos_lth[drawchr][ord][j]*row_unit+drawst,"npc"),
                                      y=unit(markerPosition+col_unit*k,"npc"),
                                      just=c("center","bottom"),
                                      gp=gpar(cex=markerFontSize,font=markerFont))
                    ...
                    if(length(traitWithEe)>0){
                        Ee_xpos[drawchr[ord][j]]<-drawst+row_unit*(pos[drawchr][ord][j]-pos_st[drawchr][ord][j])/poslth_stead[drawchr][ord][j]*pos_lth[drawchr][ord][j]
                        Ee_ypos[drawchr[ord][j]]<-col_unit*k
                    }
                    if(!is.na(Qe_Dpos[1])){
                        grid.rect(x=unit(drawst+radii+row_unit*(pos[drawchr][ord][j]-pos_st[drawchr][ord][j])/poslth_stead[drawchr][ord][j]*pos_lth[drawchr][ord][j],"npc"),
                                  y=unit(col_unit*k,"npc"),
                                  height=unit(chrWidth,"npc"),
                                  width=unit(chrWidth,"npc"),just="left",
                                  gp=gpar(fill=col_D[drawchr][ord][j],col=col_D[drawchr][ord][j]))
                        if(length(trait)>1){
                            grid.text(traitinfo[drawchr][ord][j],
                                      x=unit(drawst+radii+row_unit*(pos[drawchr][ord][j]-pos_st[drawchr][ord][j])/poslth_stead[drawchr][ord][j]*pos_lth[drawchr][ord][j],"npc"),
                                      y=unit(col_unit*k,"npc"),just="left",
                                      gp=gpar(col="white"))
                        }
                    }
          }
    ...
    if(length(traitWithEe)>0){
        Ee_x<-Ee_y<-0
        for(i in 1:length(check_Eechr)){
            Ee_x[i]<-Ee_xpos[match(check_Eechr[i],uniq_chr)]
            Ee_y[i]<-Ee_ypos[match(check_Eechr[i],uniq_chr)]
        }
        for(i in 1:(length(Ee_x)/2)){
              if(Ee_y[i]==Ee_y[i+(length(Ee_y)/2)]){
                  grid.lines(x=unit(c(Ee_x[i],Ee_x[i]),"npc"),
                             if(showMarker=="Yes") y=unit(c(Ee_y[i],Ee_y[i]+markerPosition+0.005),"npc")else y=unit(c(Ee_y[i],Ee_y[i]+0.005),"npc"),
                             gp=gpar(col=col_AA[i],lty="dotted",lwd=epiLineWidth))
                  grid.lines(x=unit(c(Ee_x[i],Ee_x[i+(length(Ee_x)/2)]),"npc"),
                             if(showMarker=="Yes") y=unit(c(Ee_y[i]+markerPosition+0.005,Ee_y[i]+markerPosition+0.005),"npc")else y=unit(c(Ee_y[i]+0.005,Ee_y[i]+0.005),"npc"),
                             gp=gpar(col=col_AA[i],lty="dotted",lwd=epiLineWidth))
                  grid.lines(x=unit(c(Ee_x[i+(length(Ee_x)/2)],Ee_x[i+(length(Ee_x)/2)]),"npc"),
                             if(showMarker=="Yes") y=unit(c(Ee_y[i]+markerPosition+0.005,Ee_y[i]),"npc")else y=unit(c(Ee_y[i]+0.005,Ee_y[i]),"npc"),
                             gp=gpar(col=col_AA[i],lty="dotted",lwd=epiLineWidth))
              }else{
                  grid.lines(x=unit(c(Ee_x[i],Ee_x[i+(length(Ee_x)/2)]),"npc"),
                         y=unit(c(Ee_y[i],Ee_y[i+(length(Ee_y)/2)]),"npc"),
                         gp=gpar(col=col_AA[i],lty="dotted",lwd=epiLineWidth))
              }
        }
        if(!is.na(Qe_Dpos[1])){
            Ee_xD<-Ee_x+radii+chrWidth/2
            #AD
            for(i in 1:(length(Ee_x)/2)){
              if(Ee_y[i]==Ee_y[i+(length(Ee_y)/2)]){
                  grid.lines(x=unit(c(Ee_x[i],Ee_x[i]),"npc"),
                             if(showMarker=="Yes") y=unit(c(Ee_y[i],Ee_y[i]+markerPosition+0.01),"npc")else y=unit(c(Ee_y[i],Ee_y[i]+0.01),"npc"),
                             gp=gpar(col=col_AD[i],lty="dotted",lwd=epiLineWidth))
                  grid.lines(x=unit(c(Ee_x[i],Ee_xD[i+(length(Ee_x)/2)]),"npc"),
                             if(showMarker=="Yes") y=unit(c(Ee_y[i]+markerPosition+0.01,Ee_y[i]+markerPosition+0.01),"npc")else y=unit(c(Ee_y[i]+0.01,Ee_y[i]+0.01),"npc"),
                             gp=gpar(col=col_AD[i],lty="dotted",lwd=epiLineWidth))
                  grid.lines(x=unit(c(Ee_xD[i+(length(Ee_x)/2)],Ee_xD[i+(length(Ee_x)/2)]),"npc"),
                             if(showMarker=="Yes") y=unit(c(Ee_y[i]+markerPosition+0.01,Ee_y[i]),"npc")else y=unit(c(Ee_y[i]+0.01,Ee_y[i]),"npc"),
                             gp=gpar(col=col_AD[i],lty="dotted",lwd=epiLineWidth))
              }else{
                  grid.lines(x=unit(c(Ee_x[i],Ee_xD[i+(length(Ee_x)/2)]),"npc"),
                         y=unit(c(Ee_y[i],Ee_y[i+(length(Ee_y)/2)]),"npc"),
            ...
            for(i in 1:(length(Ee_x)/2)){
              if(Ee_y[i]==Ee_y[i+(length(Ee_y)/2)]){
                  grid.lines(x=unit(c(Ee_xD[i],Ee_xD[i]),"npc"),
                             if(showMarker=="Yes") y=unit(c(Ee_y[i],Ee_y[i]+markerPosition+0.015),"npc")else y=unit(c(Ee_y[i],Ee_y[i]+0.015),"npc"),
                             gp=gpar(col=col_AD[i],lty="dotted",lwd=epiLineWidth))
                  grid.lines(x=unit(c(Ee_xD[i],Ee_x[i+(length(Ee_x)/2)]),"npc"),
                             if(showMarker=="Yes") y=unit(c(Ee_y[i]+markerPosition+0.015,Ee_y[i]+markerPosition+0.015),"npc")else y=unit(c(Ee_y[i]+0.015,Ee_y[i]+0.015),"npc"),
                             gp=gpar(col=col_AD[i],lty="dotted",lwd=epiLineWidth))
                  grid.lines(x=unit(c(Ee_x[i+(length(Ee_x)/2)],Ee_x[i+(length(Ee_x)/2)]),"npc"),
                             if(showMarker=="Yes") y=unit(c(Ee_y[i]+markerPosition+0.015,Ee_y[i]),"npc")else y=unit(c(Ee_y[i]+0.015,Ee_y[i]),"npc"),
                             gp=gpar(col=col_AD[i],lty="dotted",lwd=epiLineWidth))
              }else{
                  grid.lines(x=unit(c(Ee_xD[i],Ee_x[i+(length(Ee_x)/2)]),"npc"),
                         y=unit(c(Ee_y[i],Ee_y[i+(length(Ee_y)/2)]),"npc"),
            ...
            for(i in 1:(length(Ee_x)/2)){
              if(Ee_y[i]==Ee_y[i+(length(Ee_y)/2)]){
                  grid.lines(x=unit(c(Ee_xD[i],Ee_xD[i]),"npc"),
                             if(showMarker=="Yes") y=unit(c(Ee_y[i],Ee_y[i]+markerPosition+0.02),"npc")else y=unit(c(Ee_y[i],Ee_y[i]+0.02),"npc"),
                             gp=gpar(col=col_AD[i],lty="dotted",lwd=epiLineWidth))
                  grid.lines(x=unit(c(Ee_xD[i],Ee_xD[i+(length(Ee_x)/2)]),"npc"),
                             if(showMarker=="Yes") y=unit(c(Ee_y[i]+markerPosition+0.02,Ee_y[i]+markerPosition+0.02),"npc")else y=unit(c(Ee_y[i]+0.02,Ee_y[i]+0.02),"npc"),
                             gp=gpar(col=col_AD[i],lty="dotted",lwd=epiLineWidth))
                  grid.lines(x=unit(c(Ee_xD[i+(length(Ee_x)/2)],Ee_xD[i+(length(Ee_x)/2)]),"npc"),
                             if(showMarker=="Yes") y=unit(c(Ee_y[i]+markerPosition+0.02,Ee_y[i]),"npc")else y=unit(c(Ee_y[i]+0.02,Ee_y[i]),"npc"),
                             gp=gpar(col=col_AD[i],lty="dotted",lwd=epiLineWidth))
              }else{
                  grid.lines(x=unit(c(Ee_xD[i],Ee_xD[i+(length(Ee_x)/2)]),"npc"),
                         y=unit(c(Ee_y[i],Ee_y[i+(length(Ee_y)/2)]),"npc"),
